...
import { UserProfile, QuizQuestion, LearnerProfile, QuizAttempt } from '../types';
import { PRE_TEST_QUESTIONS, TOPIC_LESSONS } from '../data/lawsOfMotionData';
...
export const PreTestPage: React.FC<PreTestPageProps> = ({ topicId, userProfile, onNavigate }) => {
  const lang = userProfile.currentLanguage || 'en';
  const isMr = lang === 'mr';
  const isHi = lang === 'hi';

  const questions: QuizQuestion[] = PRE_TEST_QUESTIONS[topicId] || PRE_TEST_QUESTIONS.force;
  ...
  const handleSubmit = () => {
    ...
    questions.forEach((q, idx) => {
      ...
      if (isCorrect) {
        score++;
        if (q.conceptKey) strongConcepts.push(q.conceptKey);
      } else {
        if (q.conceptKey) weakConcepts.push(q.conceptKey);
      }
    });
    ...
    const learnerProfile: LearnerProfile = {
      ...
      preTestScore: score,
      preTestTotal: questions.length,
      lastAssessedAt: new Date().toISOString(),
    };

    storageService.saveLearnerProfile(topicId, learnerProfile);

    const attempt: QuizAttempt = {
      id: `pre-${topicId}-${Date.now()}`,
      topicId,
      quizType: 'pretest',
      score,
    ...
    onNavigate(`/pre-test-result/${topicId}`);
  };
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-slate-100">
            ...
            {currentIndex < questions.length - 1 ? (
              ...
              >
                <span>{isMr ? 'चाचणी पूर्ण करा' : isHi ? 'जांच पूरी करें' : 'View Diagnosis'}</span>
